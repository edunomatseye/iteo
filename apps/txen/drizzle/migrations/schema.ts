import { pgTable, pgEnum, bigint, timestamp, varchar, foreignKey, text, boolean, uuid, serial, jsonb, integer, uniqueIndex, primaryKey, unique } from "drizzle-orm/pg-core"
...
export const bPRbtHvpPTydfNdB = pgTable("bPRbtHvpPTydfNdB", {
	...
	id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: ""bPRbtHvpPTydfNdB_id_seq"", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807 }),
...
export const todos = pgTable("todos", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "todos_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	title: varchar("title"),
	...
	author_id: uuid("author_id").references(() => user.id, { onDelete: "cascade", onUpdate: "cascade" } ),
});
...
(table) => {
	return {
		authenticator_userId_credentialID_pk: primaryKey({ columns: [table.userId, table.credentialID], name: "authenticator_userId_credentialID_pk"}),
